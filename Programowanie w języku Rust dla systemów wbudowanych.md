...
Embedded systems serve as the digital bridge to the physical analog world, incorporating a computing element at their core much like traditional computers. However, unlike traditional computers that prioritize interfacing with humans, embedded systems are designed to interact directly with their environment. This environmental focus means that embedded systems are typically more specialized in their computational tasks compared to the versatile nature of personal computers (PCs). For instance, while a PC manages multiple simultaneous tasks such as word processing, software updates, and media streaming to ensure a seamless user experience, an embedded system is engineered to perform a specific function, such as reading temperature sensor data and adjusting fan speeds accordingly. User interactions in embedded systems are usually minimal and straightforward, often limited to simple inputs like buttons for configuration. Embedded systems are often characterized as mission-specific computing devices due to the narrow scope of their operations. At the heart of an embedded system lies an Electronic Control Unit (ECU), typically built around a microcontroller. On one end, the system receives physical inputs from the environment through various interfaces like buttons, sensors, and switches. These inputs are processed by the microcontroller, which then controls actuator outputs such as motors, lights, screens, or speakers. While embedded systems can operate independently, they are also capable of interacting with larger systems—for example, those found in automobiles—through communication interfaces. These interfaces can be either wired, such as UART or USB, or wireless, like WiFi or Bluetooth, enabling seamless connections with other input and output devices.

**Constraints and Design Considerations**

Embedded systems are designed to operate under several stringent constraints that differentiate them from traditional non-embedded computers. Key considerations include minimizing size and weight, optimizing power efficiency, and reducing costs. For example, a game console controller must be compact and lightweight for comfortable handling by gamers, maintain sufficient battery life to operate for extended periods without recharging, and remain cost-effective given the high volume of units manufactured. These constraints necessitate meticulous design and engineering to balance functionality with efficiency and affordability, ensuring that embedded systems meet their specific application requirements without exceeding resource limitations. Numerous examples illustrate the diverse applications of embedded systems, highlighting their integral role in everyday technology. Common examples include household appliances like washing machines and refrigerators, automotive systems such as engine control units and infotainment systems, consumer electronics like smartphones and game controllers, and industrial machines used in manufacturing processes. Each of these applications leverages the specialized capabilities of embedded systems to perform dedicated tasks reliably and efficiently, demonstrating the versatility and critical importance of embedded technology in modern life.
...
Embedded systems are primarily driven by microcontrollers, which incorporate microprocessors along with additional integrated functions and peripherals, distinguishing them from standalone microprocessors. While both microcontrollers (MCUs) and microprocessors are types of integrated circuits (ICs), they differ in performance and packaging. An IC package serves as the protective housing for the silicon die, providing physical protection, facilitating heat dissipation, and enabling connections to external circuits or devices.

A microprocessor functions as the central processing unit (CPU) of a computer system, executing complex instructions and performing arithmetic and logic operations. It is a highly sophisticated, general-purpose device capable of handling multitasking operations, making it suitable for devices like desktop computers, laptops, and servers. Microprocessors often feature multiple cores and rely on external components such as RAM and ROM for data and program storage, designed to handle tasks that require significant computational power and versatility.

In contrast, microcontrollers are specialized integrated circuits designed specifically for embedded systems where control-oriented tasks are essential. Unlike microprocessors, microcontrollers integrate not only a microprocessor but also on-chip memory (such as Flash or ROM for program storage and RAM for data storage) and several peripheral interfaces, including digital inputs and outputs, timers, counters, analog-to-digital converters, and serial communication interfaces. This integration allows microcontrollers to directly interface with sensors and actuators using minimal external components. Typically, microcontrollers have lower-performance CPUs with single cores, consume significantly less power, are more cost-effective, easier to program, and smaller in size compared to microprocessors. These attributes make microcontrollers ideal for a wide range of embedded applications, including home appliances, automotive control systems, robotics, industrial automation, and various consumer electronics.

### **System on Chip (SoC)**

A System on Chip (SoC) integrates multiple components of a computing system into a single integrated circuit (IC), encompassing not just a microprocessor but also various peripherals and specialized units. The term SoC is used flexibly, leading to some confusion in its application. Technically, a microcontroller can be considered an SoC since it combines a microprocessor with additional functions and peripherals. Manufacturers like Espressif refer to their ESP devices as SoCs, blurring the lines between traditional microcontrollers and SoCs. On the other hand, companies such as Intel use the term SoC to describe devices that integrate a microprocessor and chipset within a single package, typically without including main memory.

In this context, SoCs can be viewed as enhanced microcontrollers, often featuring higher performance and multiple cores. Additionally, SoCs may incorporate specialized units like graphics acceleration units or Field-Programmable Gate Array (FPGA) fabrics to boost performance and expand functionality. This makes SoCs more versatile and capable of handling complex tasks compared to standard microcontrollers. Essentially, SoCs mimic the structure of microcontrollers but offer enhanced capabilities, making them suitable for more demanding embedded applications.
...
### ProgrammingTimers & Counters (std)
...
Configuring SNTP on ESP devices using Rust involves a structured approach that begins with initializing peripherals and establishing a WiFi connection. Utilizing the `esp-idf-svc` crate, developers can instantiate and configure the `EspSntp` abstraction to handle time synchronization. By creating an SNTP instance with default settings or customizing it as needed, the device can communicate with NTP servers to obtain accurate UTC time. Monitoring the synchronization status ensures that the system clock is accurately set, which is essential for time-sensitive applications within the IoT ecosystem. Implementing SNTP enhances the reliability and coordination of networked devices by maintaining consistent and accurate time references across the system.

### The Embassy Framework (no-std)

As we progressed through this book, you might have noticed a few patterns—even notable code verbosity—that could benefit from some enhancement. One such pattern, as you recall from the ADCs section, was **blocking operations**. In a blocking approach, the processor sits idle, busy waiting (continuously polling) for a result. This not only wastes processing power but also prevents other code from executing concurrently. A potential solution to this issue is using **interrupts**, which can notify the application when an event occurs. However, configuring interrupts can be daunting and increases code verbosity.
...
The Embassy framework introduces an efficient and elegant approach to asynchronous programming in embedded Rust development. By leveraging async/await syntax and providing a lightweight runtime, Embassy allows developers to write concurrent code that is both readable and maintainable. Its integration with community-driven HALs ensures compatibility and portability across various ESP devices, making it a promising tool for the future of embedded development. Understanding and utilizing Embassy’s async abstractions and synchronization primitives can significantly enhance the performance and responsiveness of embedded applications within the IoT ecosystem.

### Synchronization Primitives

When introducing interrupts earlier in this book, dealing with global variables presented significant challenges. The primary issue stems from ensuring that variables are shared safely among threads to prevent synchronization problems like data races. However, using the Embassy framework, this experience is greatly improved through several synchronization primitives provided by the `embassy-sync` crate.
...
*Note: Waker primitives will not be covered in this book.*
...
Synchronization in concurrent and asynchronous environments is critical to ensure data integrity and prevent race conditions. The Embassy framework, through its `embassy-sync` crate, provides robust synchronization primitives tailored for embedded Rust development.

- **`AtomicU32`**: Suitable for simple, small-sized data sharing without complex synchronization needs.
- **Blocking `Mutex`**: Ideal for sharing larger data types or when more complex synchronization is required, ensuring safe access across multiple tasks.

By leveraging these primitives, developers can write safe and efficient concurrent code, minimizing synchronization issues and enhancing the reliability of embedded applications within the IoT ecosystem.

In the next sections, we will explore additional synchronization primitives provided by Embassy, such as `Channel`, `PubSubChannel`, `Signal`, and `Pipe`, each catering to specific use cases and communication patterns among tasks.
...
static SHARED: Mutex<embassy_sync::mutex::raw::CriticalSectionRawMutex, u32> = Mutex::new(0);
...
static SHARED: Signal<embassy_sync::mutex::raw::CriticalSectionRawMutex, u32> = Signal::new();
...
static SHARED: Channel<embassy_sync::mutex::raw::CriticalSectionRawMutex, u32, 2> = Channel::new();
...
Synchronization in concurrent and asynchronous environments is crucial for ensuring data integrity and preventing race conditions. The **Embassy framework**, through its `embassy-sync` crate, provides a robust set of synchronization primitives tailored for embedded Rust development.

**Key Primitives:**
- **Atomic Types (`AtomicU32`)**: Ideal for simple, small-sized data sharing without complex synchronization needs.
- **Mutex (`Mutex`)**: Suitable for sharing larger data types or when more complex synchronization is required.
- **Signal (`Signal`)**: Perfect for notifying tasks about specific events or data changes.
- **Channel (`Channel`)**: Enables buffered, first-come-first-served message passing between multiple producers and a single consumer.
- **PubSubChannel (`PubSubChannel`)**: Facilitates broadcast communication where each message is received by all subscribers.

**Choosing the Right Primitive:**
- **Simple Data Sharing**: Use atomic types for minimalistic and low-overhead synchronization.
- **Complex Data Sharing**: Use `Mutex` for safe access to shared, mutable data across tasks.
- **Event Notification**: Use `Signal` to notify tasks of specific events or data updates.
- **Message Passing**: Use `Channel` or `PubSubChannel` for structured communication between tasks, depending on whether messages need to be delivered to single or multiple consumers.

By leveraging these synchronization primitives, developers can write safe, efficient, and maintainable concurrent code for embedded applications, minimizing synchronization issues and enhancing the reliability of systems within the IoT ecosystem.
